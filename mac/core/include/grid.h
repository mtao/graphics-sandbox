...
class GridBase{
    ...
    typedef typename _dt::scalar_types<Scalar>::Vec Vec;
    ...
    protected:
    GridBase(const Vec & origin, const Veci & dim, const Vec & dx): m_origin(origin), m_N(dim), m_dx(dx), m_data(N(0)*N(1)*N(2)) {}
    ...
    Vec indexToWorld(const Veci & v) {return v.cast<Scalar>().cwiseProduct(dx()) + origin();}
